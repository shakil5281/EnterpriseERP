...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "ERP Attendance Service", Version = "v1" });
...
var attendanceCorsOrigins = builder.Configuration.GetSection("Cors:AllowedOrigins").Get<string[]>() ?? Array.Empty<string>();
        ...
        policy.SetIsOriginAllowed(origin =>
            {
                ...
                if (attendanceCorsOrigins.Length > 0)
                {
                    return attendanceCorsOrigins.Contains(origin, StringComparer.OrdinalIgnoreCase);
                }
...
app.UseSwagger();
app.UseSwaggerUI(o => o.SwaggerEndpoint("/swagger/v1/swagger.json", "Attendance v1"));
app.UseCors("default");
