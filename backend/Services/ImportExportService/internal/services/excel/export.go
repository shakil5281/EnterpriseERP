...
func ExportEmployeesExcel(rows []EmployeeExportRow) (*excelize.File, error) {
	...
	f.SetActiveSheet(0)
	headers := []string{"EmployeeCode", "EmployeeName", "Department", "Designation", "JoinDate", "Status", "GrossSalary"}
	...
	for ri, row := range rows {
		r := ri + 2
		_ = f.SetCellValue(sheet, fmt.Sprintf("A%d", r), row.EmployeeCode)
		...
		_ = f.SetCellValue(sheet, fmt.Sprintf("E%d", r), row.JoinDate)
		_ = f.SetCellValue(sheet, fmt.Sprintf("F%d", r), row.Status)
		_ = f.SetCellValue(sheet, fmt.Sprintf("G%d", r), row.GrossSalary)
	}
	for i := range headers {
...
func ExportPayrollExcel(rows []PayrollExportRow) (*excelize.File, error) {
	...
	f.SetActiveSheet(0)
	headers := []string{
		"EmployeeCode", "EmployeeName", "Department", "Designation", "GrossSalary", "BasicSalary",
