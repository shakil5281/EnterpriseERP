...
public sealed class RedisCacheService : IRedisCacheService
{
    ...
    public RedisCacheService(IConfiguration configuration, ILogger<RedisCacheService> logger)
    {
        ...
        catch (RedisConnectionException ex) { logger.LogWarning(ex, "Redis is unavailable. AccountsService will run without distributed cache."); }
    }
    ...
    public Task SetAsync<T>(string key, T value, TimeSpan expiration, CancellationToken cancellationToken = default) =>
        _database is null ? Task.CompletedTask : _database.StringSetAsync(key, JsonSerializer.Serialize(value), expiration);

    public Task RemoveAsync(string key, CancellationToken cancellationToken = default) =>
        _database is null ? Task.CompletedTask : _database.KeyDeleteAsync(key);
}
