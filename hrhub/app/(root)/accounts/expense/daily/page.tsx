...
export default function DailyExpensePage() {
    ...
    const handleExportExcel = () => {
        accountService.exportReportExcel("Payment")
            .then((response) => {
                const url = window.URL.createObjectURL(new Blob([response.data]))
                const link = document.createElement("a")
                link.href = url
                link.setAttribute("download", `Daily_Expense_Report_${filters.date || "Export"}.xlsx`)
    ...
    const handleExportPdf = () => {
        accountService.exportReportPdf("Payment")
            .then((response) => {
                const url = window.URL.createObjectURL(new Blob([response.data]))
                const link = document.createElement("a")
                link.href = url
                link.setAttribute("download", `Daily_Expense_Report_${filters.date || "Export"}.pdf`)
                document.body.appendChild(link)
                link.click()
                link.remove()
                window.URL.revokeObjectURL(url)
    ...
    return (
        <div className="flex-1 space-y-6 p-8 pt-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight">Daily Expense</h2>
                    <p className="text-muted-foreground">Comprehensive daily audit of all outward financial flows</p>
                </div>
                <div className="flex gap-2">
                    ...
                    <Button size="sm" className="gap-2 bg-rose-600 hover:bg-rose-700 shadow-sm border-rose-500" onClick={handleExportExcel}>
                        <IconDownload className="h-4 w-4" />
                        Export Excel
                    </Button>
                </div>
            ...
            {showFilters && (
                <Card className="border-none shadow-sm bg-muted/20">
                    <CardHeader className="pb-3 flex flex-row items-center justify-between space-y-0">
                        <CardTitle className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Advanced Audit Filters</CardTitle>
                        <Button
                            variant="ghost"
                            size="sm"
                    ...
                    <CardContent>
                        <div className="grid gap-4 md:grid-cols-5">
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-bold uppercase text-muted-foreground">Quick Search</Label>
                            ...
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-bold uppercase text-muted-foreground">Branch</Label>
                                <NativeSelect
                                    value={filters.branchId}
                                    onChange={(e) => setFilters({ ...filters, branchId: e.target.value })}
                                    className="h-9 text-xs"
                                >
                                    <NativeSelectOption value="all">Every Branch</NativeSelectOption>
                                    {branches.map(b => (
                            ...
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-bold uppercase text-muted-foreground">Fund Source</Label>
                                <NativeSelect
                            ...
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-bold uppercase text-muted-foreground">Sector</Label>
                                <NativeSelect
                                    value={filters.category}
                                    onChange={(e) => setFilters({ ...filters, category: e.target.value })}
                                    className="h-9 text-xs"
                                >
                                    <NativeSelectOption value="all">All Sectors</NativeSelectOption>
                                    {categories.map(c => (
            ...
            <div className="grid gap-6 md:grid-cols-4">
                <Card className="md:col-span-1 border-l-4 border-l-rose-500 shadow-sm">
                    <CardContent className="pt-6">
                        <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">Expenditures</div>
                        <div className="text-2xl font-bold">{filteredData.length}</div>
                        <p className="text-[10px] text-muted-foreground mt-1 font-medium">Recorded today</p>
                    </CardContent>
                </Card>

                <Card className="md:col-span-3 border-l-4 border-l-rose-600 shadow-sm">
                    <div className="p-6 flex justify-between items-center">
                        <div>
                            <p className="text-[10px] font-bold uppercase tracking-wider text-rose-600 mb-1">Total Funds Outward</p>
                            <p className="text-4xl font-bold text-rose-700 tracking-tighter">
                                {formatCurrency(total)}
                            </p>
                        </div>
                        <div className="p-4 bg-rose-500/10 rounded-xl border border-rose-500/20">
                            <IconReceipt2 className="h-8 w-8 text-rose-600" />
                        </div>
                    </div>
                </Card>
            </div>
            ...
            <Card className="shadow-sm border-none bg-card ring-1 ring-border">
                <CardHeader className="flex flex-row items-center justify-between border-b bg-muted/10 py-4">
                    <CardTitle className="text-lg font-bold flex items-center gap-2">
                        <IconCalendarSearch className="h-5 w-5 text-rose-600" />
                        Expenditure Register
                    </CardTitle>
                    {selectedRows.length > 0 && (
                        <div className="flex items-center gap-3 bg-rose-50 dark:bg-rose-950/30 px-3 py-1.5 rounded-lg border border-rose-200 dark:border-rose-800">
                            <span className="text-xs font-bold text-rose-700 dark:text-rose-400">{selectedRows.length} items selected</span>
                            <div className="h-3 w-px bg-rose-200 dark:bg-rose-800" />
                ...
                <CardContent className="p-0 overflow-auto">
                    <Table>
                        ...
                        <TableBody>
                            {loading ? (
                                <TableRow>
                                    <TableCell colSpan={9} className="text-center py-24 italic text-muted-foreground animate-pulse">
                                        Retrieving expenditure journal...
                                    </TableCell>
                                </TableRow>
                            ) : filteredData.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={9} className="text-center py-24 text-muted-foreground font-medium">
                                ...
                                filteredData.map((tx, index) => (
                                    <TableRow key={tx.id || index} className="group hover:bg-muted/5 transition-colors">
                                        ...
                                        <TableCell className="text-xs font-medium text-center whitespace-nowrap">
                                            {tx.transactionDate?.split('T')[0]}
                                        </TableCell>
                                        <TableCell className="font-mono text-[10px] font-bold text-rose-600 whitespace-nowrap">
                                        ...
                                        <TableCell>
                                            <span className="text-xs font-medium text-muted-foreground line-clamp-1 max-w-[200px]">{tx.description || "-"}</span>
                                        </TableCell>
                                        <TableCell className="text-right font-bold text-rose-600 font-mono text-sm">
