...
import { rosterService } from "@/lib/services/roster"
...
export default function RosterAssignPage() {
    ...
    // selection state
    const [selectedEmployees, setSelectedEmployees] = React.useState<EmployeeSimple[]>([])
    ...
    const handleApplyFilters = async () => {
        ...
        try {
            ...
            const data = await employeeService.getEmployeesSimple(params)
            setEmployees(data)
    ...
    const handleSubmitRoster = async () => {
        ...
        try {
            ...
            toast.success(`${selectedEmployees.length} employees scheduled successfully from ${startDate} to ${endDate}`)
            router.push('/management/human-resource/roster')
        } catch (error) {
            toast.error("Failed to process roster. Please ensure all dates are valid.")
        } finally {
    ...
    const columns: ColumnDef<EmployeeSimple>[] = [
        ...
        },
        {
            ...
            header: "Department",
            cell: ({ row }) => <Badge variant="outline">{row.original.departmentName}</Badge>
        },
        {
        ...
        <div className="flex flex-col gap-6 py-6 bg-muted/20 min-h-screen px-4 lg:px-8 w-full max-w-[1600px] mx-auto text-foreground">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Schedule Roster</h1>
                        <p className="text-sm text-muted-foreground font-medium">Assign shifts to multiple employees across flexible dates.</p>
                    </div>
                </div>
            </div>
            ...
            <Card className="border-none shadow-sm overflow-hidden bg-background">
                ...
                <CardContent className="p-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-y-6 gap-x-4">
                        ...
                        <div className="space-y-2">
                            <Label className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider ml-1">Shift</Label>
                        ...
                        <div className="flex items-end">
                            <Button className="w-full h-10 bg-emerald-600 hover:bg-emerald-700 text-white gap-2 shadow-lg shadow-emerald-600/20 font-semibold rounded-xl transition-all" onClick={handleApplyFilters} disabled={isLoadingEmployees}>
                                {isLoadingEmployees ? <IconLoader className="size-4 animate-spin" /> : <IconSearch className="size-4" />}
                                Apply Filters
                            </Button>
                        </div>
                    </div>
            ...
            {/* Roster Assignment Section (Moved to uper data table) */}
            <Card className={cn(
                ...
                <CardHeader className="border-b pb-6 px-6 bg-muted/5">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <CardTitle className="text-xl font-bold italic tracking-tight uppercase">Employee Pool</CardTitle>
                                <CardDescription className="text-xs font-semibold">Select personnel for synchronization.</CardDescription>
                            </div>
                        </div>
                    </div>
                </CardHeader>
