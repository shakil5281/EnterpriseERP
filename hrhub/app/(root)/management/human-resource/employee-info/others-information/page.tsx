...
const selectedAddressId = (value: string) =>
  value === "0" ? undefined : value;

export default function OthersInformationPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const employeeIdUrl = searchParams.get("employeeId");
  ...
  const [presentPostOffices, setPresentPostOffices] = React.useState<
    PostOffice[]
  >([]);

  const [permanentDistricts, setPermanentDistricts] = React.useState<
    District[]
  >([]);
  const [permanentThanas, setPermanentThanas] = React.useState<Thana[]>([]);
  const [permanentPostOffices, setPermanentPostOffices] = React.useState<
    PostOffice[]
  >([]);
  ...
  React.useEffect(() => {
    if (employee?.presentDivisionId) {
      addressService
        .getDistricts(String(employee.presentDivisionId))
  ...
  React.useEffect(() => {
    if (employee?.presentDistrictId) {
      addressService
        .getThanas(String(employee.presentDistrictId))
        .then(setPresentThanas)
        .catch(console.error);
      addressService
        .getPostOffices({ districtId: String(employee.presentDistrictId) })
        .then(setPresentPostOffices)
        .catch(console.error);
    } else {
      setPresentThanas([]);
  ...
  React.useEffect(() => {
    if (employee?.permanentDivisionId) {
      addressService
        .getDistricts(String(employee.permanentDivisionId))
        .then(setPermanentDistricts)
        .catch(console.error);
    } else {
  ...
  React.useEffect(() => {
    if (employee?.permanentDistrictId) {
      addressService
        .getThanas(String(employee.permanentDistrictId))
        .then(setPermanentThanas)
        .catch(console.error);
      addressService
        .getPostOffices({ districtId: String(employee.permanentDistrictId) })
        .then(setPermanentPostOffices)
        .catch(console.error);
    } else {
      setPermanentThanas([]);
  ...
  const loadEmployee = React.useCallback(
    async (empId: string, compId: number) => {
      setIsLoading(true);
      try {
        const data = await employeeService.getEmployee(empId, compId);
        if (data) {
          setEmployee(data);
        } else {
  ...
  const loadEmployeeById = React.useCallback(async (id: number) => {
    setIsLoading(true);
    try {
  ...
  React.useEffect(() => {
    if (employeeIdUrl && companyIdUrl) {
      loadEmployee(employeeIdUrl, parseInt(companyIdUrl));
    } else if (idUrl) {
      if (companyIdUrl) {
        // If companyId is present, try literal EmployeeId string first
        loadEmployee(idUrl, parseInt(companyIdUrl));
      } else {
        // Otherwise treat as internal integer ID
        const numericId = parseInt(idUrl);
        if (!isNaN(numericId)) {
          loadEmployeeById(numericId);
        } else {
          toast.error("Invalid Employee ID format");
        }
      }
    }
  }, [employeeIdUrl, companyIdUrl, idUrl, loadEmployee, loadEmployeeById]);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // Search across all employees to find the right one (returns EmployeeSimple)
      const results = await employeeService.getEmployeesSimple({
        searchTerm: searchId.trim(),
        isActive: undefined,
      });

      const foundEmployee = results.find(
        (e) =>
          e.employeeId === searchId ||
          e.id.toString() === searchId ||
          e.fullNameEn.toLowerCase().includes(searchId.toLowerCase()),
      );

      if (foundEmployee) {
        // Use internal ID and Company ID for redirection
        router.push(
          `/management/human-resource/employee-info/others-information?id=${foundEmployee.id}&companyId=${foundEmployee.companyId}`,
        );
      } else {
        toast.error("Employee not found");
      }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Collect all data from form and merge with existing employee data
      const data: UpdateEmployeeDto = {
        ...employee,
        // Personal/Basic
        proximity: (formData.get("proximity") as string) || employee.proximity,
        religion: (formData.get("religion") as string) || employee.religion,
        gender: (formData.get("gender") as string) || employee.gender,
        bloodGroup:
          (formData.get("bloodGroup") as string) || employee.bloodGroup,

        // Address
        presentAddress: formData.get("presentAddress") as string,
        presentAddressBn: formData.get("presentAddressBn") as string,
        presentDivisionId: optionalFormId(
          formData,
          "presentDivisionId",
          employee.presentDivisionId,
        ),
        presentDistrictId: optionalFormId(
          formData,
          "presentDistrictId",
          employee.presentDistrictId,
        ),
        presentThanaId: optionalFormId(
          formData,
          "presentThanaId",
          employee.presentThanaId,
        ),
        presentPostOfficeId: optionalFormId(
          formData,
          "presentPostOfficeId",
          employee.presentPostOfficeId,
        ),
        presentPostalCode:
          (formData.get("presentPostalCode") as string) ||
        ...
        permanentDivisionId: optionalFormId(
          formData,
          "permanentDivisionId",
          employee.permanentDivisionId,
        ),
        permanentDistrictId: optionalFormId(
          formData,
          "permanentDistrictId",
          employee.permanentDistrictId,
        ),
        permanentThanaId: optionalFormId(
          formData,
          "permanentThanaId",
          employee.permanentThanaId,
        ),
        permanentPostOfficeId: optionalFormId(
          formData,
          "permanentPostOfficeId",
          employee.permanentPostOfficeId,
        ),
        permanentPostalCode:
          (formData.get("permanentPostalCode") as string) ||
        ...
        // Salary (Keep current employee values as they are calculated by handleGrossSalaryChange)
        ...
        emergencyContactAddress: formData.get(
      ...
      loadEmployee(employee.employeeId, employee.companyId || 0);
    } catch (error: any) {
      toast.error(
  ...
  if (!employeeIdUrl && !idUrl && !employee) {
    return (
      <div className="flex flex-col gap-6 p-4 lg:p-6 mx-auto w-full max-w-2xl">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Enter Employee ID or Name to manage their additional information.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              ID: {employee?.employeeId} | {employee?.designationName} |{" "}
        ...
        <div className="flex gap-2">
          <Button
            ...
            onClick={() => {
              setEmployee(null);
              router.push(
                "/management/human-resource/employee-info/others-information",
              );
            }}
          >
            Change Employee
            ...
            <Card className="border-l-4 border-l-primary shadow-md">
              ...
              <CardContent className="grid gap-6 pt-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input
                      id="fatherNameEn"
                      name="fatherNameEn"
                      defaultValue={employee?.fatherNameEn}
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <Input
                      id="fatherNameBn"
                      name="fatherNameBn"
                      defaultValue={employee?.fatherNameBn}
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input
                      id="motherNameEn"
                      name="motherNameEn"
                      defaultValue={employee?.motherNameEn}
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <Input
                      id="motherNameBn"
                      name="motherNameBn"
                      defaultValue={employee?.motherNameBn}
                      className="font-sutonny"
                    />
                  </div>
                  <div className="grid gap-2">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input
                      id="proximity"
                      name="proximity"
                      defaultValue={employee?.proximity}
                    />
                  </div>
                  <div className="grid gap-2">
                ...
                <div className="pt-4 border-t space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      <Label htmlFor="spouseNameEn">
                        Spouse Name (English)
                      </Label>
                      <Input
                        id="spouseNameEn"
                        name="spouseNameEn"
                        defaultValue={employee?.spouseNameEn}
                      />
                    </div>
                    <div className="grid gap-2">
                      ...
                      <Input
                        id="spouseNameBn"
                        name="spouseNameBn"
                        defaultValue={employee?.spouseNameBn}
                        className="font-sutonny"
                      />
                    </div>
                    <div className="grid gap-2">
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input
                        id="spouseContact"
                        name="spouseContact"
                        defaultValue={employee?.spouseContact}
                      />
                    </div>
                  </div>
                </div>
              ...
              <CardContent className="grid gap-6 pt-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="grid gap-2 sm:col-span-2 lg:col-span-3">
                      <Label htmlFor="presentAddressBn">
                        Address Line (Bangla)
                      </Label>
                      <Textarea
                        id="presentAddressBn"
                        name="presentAddressBn"
                        defaultValue={employee?.presentAddressBn}
                        placeholder="বাড়ি/সড়ক/গ্রাম ইত্যাদি"
                        className="font-sutonny"
                      />
                    </div>
                        ...
                        onChange={(e) =>
                          setEmployee((prev) =>
                            prev
                              ? {
                                  ...prev,
                                  presentDivisionId: selectedAddressId(
                                    e.target.value,
                                  ),
                                }
                              : null,
                        ...
                        onChange={(e) =>
                          setEmployee((prev) =>
                            prev
                              ? {
                                  ...prev,
                                  presentDistrictId: selectedAddressId(
                                    e.target.value,
                                  ),
                                }
                              : null,
                        ...
                        onChange={(e) =>
                          setEmployee((prev) =>
                            prev
                              ? {
                                  ...prev,
                                  presentThanaId: selectedAddressId(
                                    e.target.value,
                                  ),
                                }
                              : null,
                    ...
                    <div className="grid gap-2">
                      ...
                      <NativeSelect
                        ...
                        onChange={(e) => {
                          const id = selectedAddressId(e.target.value);
                          const po = presentPostOffices.find(
                            (p) => p.id === id,
                          );
                          setEmployee((prev) =>
                            prev
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        value={employee?.presentPostalCode || ""}
                        onChange={(e) =>
                          setEmployee((prev) =>
                            prev
                              ? { ...prev, presentPostalCode: e.target.value }
                              : null,
                          )
                        }
                      />
                ...
                <div className="space-y-4 pt-4 border-t">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="grid gap-2 sm:col-span-2 lg:col-span-3">
                      <Label htmlFor="permanentAddressBn">
                        Address Line (Bangla)
                      </Label>
                      <Textarea
                        id="permanentAddressBn"
                        name="permanentAddressBn"
                        defaultValue={employee?.permanentAddressBn}
                        placeholder="বাড়ি/সড়ক/গ্রাম ইত্যাদি"
                        className="font-sutonny"
                      />
                    </div>
                        ...
                        onChange={(e) =>
                          setEmployee((prev) =>
                            prev
                              ? {
                                  ...prev,
                                  permanentDivisionId: selectedAddressId(
                                    e.target.value,
                                  ),
                                }
                              : null,
                        ...
                        onChange={(e) =>
                          setEmployee((prev) =>
                            prev
                              ? {
                                  ...prev,
                                  permanentDistrictId: selectedAddressId(
                                    e.target.value,
                                  ),
                                }
                              : null,
                        ...
                        onChange={(e) =>
                          setEmployee((prev) =>
                            prev
                              ? {
                                  ...prev,
                                  permanentThanaId: selectedAddressId(
                                    e.target.value,
                                  ),
                                }
                              : null,
                    ...
                    <div className="grid gap-2">
                      ...
                      <NativeSelect
                        ...
                        onChange={(e) => {
                          const id = selectedAddressId(e.target.value);
                          const po = permanentPostOffices.find(
                            (p) => p.id === id,
                          );
                          setEmployee((prev) =>
                            prev
                              ? {
                                  ...prev,
                                  permanentPostOfficeId: id,
                                  permanentPostalCode:
                                    po?.postalCode ||
                                    po?.code ||
                                    prev.permanentPostalCode,
                                }
                              : null,
                          );
                        }}
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        value={employee?.permanentPostalCode || ""}
                        onChange={(e) =>
                          setEmployee((prev) =>
                            prev
                              ? { ...prev, permanentPostalCode: e.target.value }
                              : null,
                          )
                        }
                      />
              ...
              <CardHeader className="bg-linear-to-r from-primary/5 to-transparent">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription>
                      Salary structure and allowances (Calculated automatically
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="grid gap-2 border-b pb-2 sm:col-span-2 lg:col-span-3 mb-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Enter the Gross Salary to automatically calculate the
                      breakdown below.
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <NativeSelect
                      ...
                      name="bankName"
                      defaultValue={
                        employee?.bankName || "Islami Bank Bangladesh Limited"
                      }
                    >
                      <option value="Islami Bank Bangladesh Limited">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input
                      id="bankRoutingNo"
                      name="bankRoutingNo"
                      defaultValue={employee?.bankRoutingNo}
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <NativeSelect
                      ...
                      defaultValue={employee?.bankAccountType || "Savings"}
                    >
              ...
              <CardContent className="flex flex-col items-center justify-center py-10">
                ...
                >
                  {employee?.signatureImageUrl ? (
                    <img
                      src={`${getPublicApiOrigin()}${employee.signatureImageUrl}`}
                      alt="Signature"
