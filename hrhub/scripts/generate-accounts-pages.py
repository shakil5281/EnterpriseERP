#!/usr/bin/env python3
"""Generate hrhub accounts module page.tsx files."""
...
export default function FiscalYearsPage() {
  ...
  const create = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await accountsService.createFiscalYear({ companyId, yearName, startDate, endDate, createdBy: user?.id ?? null });
                ...
                {rows.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-right">
                      {!r.isClosed && <Button variant="outline" size="sm" onClick={() => closeYear(r.id)}>Close</Button>}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
