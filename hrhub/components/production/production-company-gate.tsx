...
type ProductionCompanyGateProps = {
  children: (companyId: string) => React.ReactNode;
};

export function ProductionCompanyGate({ children }: ProductionCompanyGateProps) {
  ...
  if (!ready || !companyId) {
    return (
      <MerchEmptyState
        variant="no-company"
        description="Choose an active company from the header to load production data."
      />
    );
  }
